...
const Group = () => {
    ...
    const checkGroupCreation = async () => {
        ...
        const response = await fetch("http://ec2-54-160-112-17.compute-1.amazonaws.com:8000/createdGroupStatus", requestOption)
            .then(async response => {
    ...
    const createGroup = async () => {
        ...
        const response = await fetch("http://ec2-54-160-112-17.compute-1.amazonaws.com:8000/createGroupSession", requestOption)
            .then(async response => {
                const data = await response.json()
                if (response.ok) {
                    console.log(data)
                                ...
                                <div className="d-flex flex-column">
                                    <h1 className="move-bold">Group Settings</h1>
                                    {
                                        groupCreationStatus == 0 ?
                                            ...
                                            :
                                            <p className="move-medium black-theme mt-3">Invite your friends: http://ec2-54-160-112-17.compute-1.amazonaws.com:3000/join/group/{groupCreationStatus}</p>
                                    }
                                </div>
                            </div>
                        </div>
